...
public class PreSpawnHordeSpawnerController : MonoBehaviour
{
    ...
    private bool isDoorOpened; //플레이어가 접근하여 문이 열림
    private bool isDespawned; //스폰해제가 된 적 있는가?
    [SerializeField] private Tile tileSpawning; //현재 타일
    ...
    // DoorTrigger 스크립트가 호출할 진입점
    public void OnPlayerDoorApproach()
    {
        if (isDoorOpened) 
            return;
        ...
        // 다른 방에 스폰된 놈들 제거
        var controllers = FindObjectsByType<PreSpawnHordeSpawnerController>(FindObjectsSortMode.None);
        foreach (var ctrl in controllers)
            ctrl.AnotherRoomTriggered();
    }

    public void AnotherRoomTriggered() //다른 방 트리거에 플레이어가 접근함.
    {
        if (!isDoorOpened) return; //내부 확인된 적 없음.
    ...
    private void ManagePlayerLocation(DungenCharacter character, Tile previousTile, Tile newTile)
    {
        ...
        if (!newTile.IsAdjacentTo(tileSpawning) && newTile != tileSpawning) //문 열린적 있음. 두칸이상 멀어짐.
        {
    ...
    private void SpawnPreSpawnHorde()
    {
        foreach (var sp in spawners)
        {
            if (sp.gameObject.activeInHierarchy)
                sp.TrySpawn(GamePlayManager.instance.currentMapIndex,false, danger);
        }
    }

    /// <summary>
    /// 다른방으로 플레이어가 가버릴 때, 내부에 남은 좀비는 제거.
    /// </summary>
