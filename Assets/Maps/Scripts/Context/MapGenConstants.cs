public static class MapGenConstants
{
    ...
    public const int MaxCreatureCountOnSpawnRoom = 3; //한 방에서 생성될 수 있는 적의 최대 수.
    ...
    public const int MaxNormalCreatureCountLimitOnStage = 50; //스테이지 내 일반 적의 수의 리미트.
    public const int BaseMaxNormalCreatureCountOnStage = 30; //하나의 스테이지에 존재할 수 있는 일반 적의 최대 수.
    public const int MaxNormalCreatureCountOffsetByStage = 2; //스테이지마다 증가하는 존재가능한 일반 적의 최대 수.

    public const int MaxBigCreatureCountLimitOnStage = 11; //스테이지 내 거대 적의 수의 리미트.
    public const int BaseMaxBigCreatureCountOnStage = 1; //하나의 스테이지에 존재할 수 있는 거대 적의 최대 수.
    public const int MaxBigCreatureCountOffsetByStage = 1; //스테이지마다 증가하는 존재가능한 거대 적의 최대 수.

    public const int MaxBombCreatureCountLimitOnStage = 5; //스테이지 내 자폭형 적의 수의 리미트.
    public const int BaseMaxBombCreatureCountOnStage = 1; //하나의 스테이지에 존재할 수 있는 자폭형 적의 최대 수.
