...
namespace MaykerStudio.VFX
{
    ...
    public class SlashDecal : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (IsGrounded(out Vector3 hit , out Vector3 hitNormal))
            {
                transform.SetPositionAndRotation(hit + decalOffsetY * Vector3.up, slash.transform.rotation);
                if (!decal.isPlaying)
                {
                    decal.Play(true);
                }
        ...
        bool IsGrounded(out Vector3 collisionPoint, out Vector3 hitNormal)
        {
